...
| Tree-shaking | 需要（减小 Bundle 体积） | 不需要（体积不重要） |
...
data-fetch-loader 在**客户端构建**时，将 `getServerSideProps`、`getStaticProps`、`getStaticPaths` 替换为空实现：
...
`NamiBuilder` 在构建前需要知道每个页面使用了哪些数据预取函数，以决定构建策略：
