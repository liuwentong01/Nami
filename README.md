...
┌─────────────────────────────────────────────────────────┐
│                      @nami/cli                          │
│          dev · build · start · generate · analyze · info │
├──────────────────────┬──────────────────────────────────┤
...
Nami 的插件系统正式生命周期主要使用 **waterfall**（串行管道）和 **parallel**（并行执行）；核心 `PluginManager` 同时预留了 **bail**（短路返回）调度能力，便于未来扩展竞争式钩子：
...
请求 →  shutdownAware    停机感知（停机中返回 503）
     ...
     →  [server.middlewares] 用户自定义 Koa 中间件
     →  [plugins]        插件中间件
     →  errorIsolation   错误隔离（防止单请求崩溃进程）
     →  isrCache         ISR 缓存层（启用 ISR 时注册，命中则直接返回）
     →  render           核心渲染（路由匹配 + 数据预取 + 渲染 + 插件 extra 消费）
→ 响应
```

`server.middlewares` 与插件中间件数量都由配置决定，`isrCache` 只在 `isr.enabled` 为 true 时注册。因此这里描述的是固定相对顺序，不是固定层数。
